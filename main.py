...
def print_config():
    ...
    if config.MODE == "market_maker":
        ...
        print(f"  Vol multiplier:  {mc.MM_VOL_MULTIPLIER}x")
        print(f"  Vol window:      {mc.MM_VOL_WINDOW} scans ({mc.MM_VOL_WINDOW * mc.MM_REQUOTE_INTERVAL}s)")
        print(f"  Vol pause:       ${mc.MM_VOL_PAUSE_THRESHOLD/100:.2f} move in {mc.MM_VOL_PAUSE_LOOKBACK * mc.MM_REQUOTE_INTERVAL}s")
